...
const FsmGraphInner: React.FC = () => {
  const { fsm, updateState, addState, deleteState, addTransition, deleteTransition, updateTransition } = useFsmStore();
    ...
    Object.values(fsm.states).map((state) => ({
      ...
      style: {
        ...
        border: state.isFinal 
          ? (selectedNodeId === state.id ? '2px solid #2563eb' : '2px double #2563eb')
          : (selectedNodeId === state.id ? '2px solid #2563eb' : '1px solid #d1d5db'),
        borderRadius: '50%',
        width: 60,
        height: 60,
        display: 'flex',
        ...
        fontSize: '12px',
        fontWeight: 'bold',
        boxShadow: state.isFinal ? '0 0 0 2px white, 0 0 0 4px #d1d5db' : 'none',
      },
    })), [fsm.states, selectedNodeId, fsm.type]);

  const edges: Edge[] = useMemo(() => {
    // Group transitions by from-to pair
    const grouped = fsm.transitions.reduce((acc, t) => {
    ...
    return Object.entries(grouped).map(([key, transitions]) => {
      ...
      const hasReverse = fsm.transitions.some(rt => rt.from === to && rt.to === from);
  ...
  const onNodesChange = useCallback((changes: any) => {
    changes.forEach((change: any) => {
      if (change.type === 'position' && change.position) {
        updateState(change.id, { x: change.position.x, y: change.position.y });
      }
    ...
    changes.forEach((change: any) => {
      ...
      if (change.type === 'select') {
        if (change.selected) {
          ...
          if (match) {
            const [, from, to] = match;
            const firstT = fsm.transitions.find(t => t.from === from && t.to === to);
            if (firstT) {
          ...
          if (match) {
            const [, from, to] = match;
            const isStillSelected = fsm.transitions.some(t => t.id === selectedEdgeId && t.from === from && t.to === to);
  ...
  const onConnect = useCallback((params: Connection) => {
    ...
    if (fsm.type === 'DFA') {
      ...
      const nextAvailable = fsm.alphabet.find(symbol => !existingInputs.includes(symbol));
      if (nextAvailable) {
        input = nextAvailable;
      }
    }
  ...
  const selectedTransition = selectedEdgeId ? fsm.transitions.find(t => t.id === selectedEdgeId) : null;
        ...
        <div className="absolute top-4 right-4 w-64 bg-white/90 backdrop-blur-sm border border-zinc-200 rounded-xl shadow-xl p-4 z-50">
          {selectedState && (
            <>
              ...
              <div className="space-y-4">
                ...
                <div className="flex gap-4">
                  ...
                  <label className="flex items-center gap-2 cursor-pointer">
                    <input 
                      ...
                      onChange={(e) => updateState(selectedState.id, { isFinal: e.target.checked })}
                      className="rounded text-blue-600 focus:ring-blue-500"
                    />
                ...
                {fsm.type === 'Moore' && (
                  <div>
                    <label className="text-[10px] font-bold text-zinc-400 uppercase">Output Symbol</label>
                    <input 
                      type="text" 
                      value={selectedState.output || ''}
                      onChange={(e) => updateState(selectedState.id, { output: e.target.value })}
                      placeholder="e.g. 1"
                      className="w-full bg-zinc-50 border border-zinc-200 rounded px-2 py-1 text-sm focus:ring-1 focus:ring-blue-500 outline-none"
                    />
                  </div>
          ...
          {selectedTransition && (
            <>
              ...
              <div className="space-y-4">
                <div>
                  <label className="text-[10px] font-bold text-zinc-400 uppercase">Input Symbol</label>
                  <input 
                    type="text" 
                    value={selectedTransition.input}
                    onChange={(e) => updateTransition(selectedTransition.id, { input: e.target.value })}
                    className="w-full bg-zinc-50 border border-zinc-200 rounded px-2 py-1 text-sm focus:ring-1 focus:ring-blue-500 outline-none"
                  />
                </div>
                ...
                {fsm.type === 'Mealy' && (
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => updateTransition(selectedTransition.id, { output: e.target.value })}
                      placeholder="e.g. 1"
                      className="w-full bg-zinc-50 border border-zinc-200 rounded px-2 py-1 text-sm focus:ring-1 focus:ring-blue-500 outline-none"
                    />
                  </div>
